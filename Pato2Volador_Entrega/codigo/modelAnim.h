...
public:
	...
	// Centro geométrico del modelo (en coords locales sin escalar).
	// Sirve como pivote de rotación para que el modelo gire sobre sí mismo
	// en vez de barrer un arco grande alrededor del origen exportado.
    ...
    ModelAnim(string const &path, bool gamma = false) : gammaCorrection(gamma)
    {
        ...
        computeAnimatedCenter();   // sobrescribe boundingCenter con el centro real animado
    }

    // Calcula el centro del pato YA con la animación aplicada (t=0).
    // Esto sí refleja dónde aparece visualmente el cuerpo, no dónde están los vértices del rest pose.
    void computeAnimatedCenter()
    {
        if (m_num_bones == 0) return; // no hay esqueleto

        // Forzar el cálculo de las matrices de huesos a t=0
        vector<aiMatrix4x4> dummy;
        boneTransform(0.0, dummy);
        // Ahora m_bone_matrices[i].final_world_transform está listo

        glm::vec3 sum(0.0f);
        glm::vec3 mn( 1e9f,  1e9f,  1e9f);
        glm::vec3 mx(-1e9f, -1e9f, -1e9f);
        size_t count = 0;

        for (auto& mesh : meshes) {
            // Muestreamos cada N vértices (no necesitamos todos para el centroide)
            size_t step = std::max((size_t)1, mesh.vertices.size() / 500);
            for (size_t i = 0; i < mesh.vertices.size(); i += step) {
                glm::vec3 restPos = mesh.vertices[i].Position;

                // Aplica las mismas transformaciones que el shader (suma ponderada de huesos)
                glm::vec3 transformedPos(0.0f);
                float totalWeight = 0.0f;
                for (int w = 0; w < NUM_BONES_PER_VEREX; w++) {
                    uint boneId   = mesh.bones_id_weights_for_each_vertex[i].ids[w];
                    float weight  = mesh.bones_id_weights_for_each_vertex[i].weights[w];
                    if (weight > 0.0f && boneId < m_num_bones) {
                        aiMatrix4x4& bm = m_bone_matrices[boneId].final_world_transform;
                        aiVector3D ap(restPos.x, restPos.y, restPos.z);
                        aiVector3D tp = bm * ap;
                        transformedPos += glm::vec3(tp.x, tp.y, tp.z) * weight;
                        totalWeight += weight;
                    }
                }
                glm::vec3 final = (totalWeight > 0.0f) ? (transformedPos / totalWeight) : restPos;
                sum += final;
                mn = glm::min(mn, final);
                mx = glm::max(mx, final);
                count++;
        ...
        if (count > 0) {
            glm::vec3 animCenter = sum / (float)count;
            std::cout << ">>> [Pato] centro ANIMADO (t=0) sample=" << count << "\n";
            std::cout << ">>> [Pato]   centroide animado = ("
                      << animCenter.x << ", " << animCenter.y << ", " << animCenter.z << ")\n";
            std::cout << ">>> [Pato]   bbox animado min = (" << mn.x << ", " << mn.y << ", " << mn.z << ")\n";
            std::cout << ">>> [Pato]   bbox animado max = (" << mx.x << ", " << mx.y << ", " << mx.z << ")\n";
            // Usamos el centro del bounding box animado, mas robusto que el centroide
            boundingCenter = (mn + mx) * 0.5f;
            std::cout << ">>> [Pato]   USADO como pivote: bbox center animado = ("
                      << boundingCenter.x << ", " << boundingCenter.y << ", " << boundingCenter.z << ")\n";
        }
    }

    void computeBoundingCenter()
    {
        glm::vec3 mn( 1e9f,  1e9f,  1e9f);
        glm::vec3 mx(-1e9f, -1e9f, -1e9f);
        glm::vec3 sum(0.0f);
        ...
        // Usamos el centroide (promedio) en vez del centro del bbox, más robusto si hay
        // vértices outliers (huesos extendidos, alas estiradas, cola larga, etc.)
        if (count > 0) boundingCenter = sum / (float)count;
        std::cout << ">>> [Pato] bbox min = (" << mn.x << ", " << mn.y << ", " << mn.z << ")\n";
        std::cout << ">>> [Pato] bbox max = (" << mx.x << ", " << mx.y << ", " << mx.z << ")\n";
        std::cout << ">>> [Pato] bbox center = ("
                  << ((mn.x+mx.x)*0.5f) << ", " << ((mn.y+mx.y)*0.5f) << ", " << ((mn.z+mx.z)*0.5f) << ")\n";
        std::cout << ">>> [Pato] centroide (usado como pivote) = ("
                  << boundingCenter.x << ", " << boundingCenter.y << ", " << boundingCenter.z << ")\n";
    }
...
private:
    ...
    MeshAnim processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for(unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // tangent/bitangent omitidos: este proyecto usa un Vertex sin esos campos
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            if(!skip)
            {   // Sanitiza la ruta: Blender FBX a veces escribe "Pato.fbm\nombre" sin extensión.
                // Tomamos solo el basename y probamos extensiones comunes.
                ...
                std::cout << ">>> [Pato] FBX pidio: '" << str.C_Str() << "' -> basename: '" << base << "'" << std::endl;

                Texture texture;
                texture.id = 0;

                // Caso 1: textura EMBEBIDA dentro del FBX (Blender "Embed Textures").
                // Assimp expone estas con el nombre "*N" donde N es el indice en scene->mTextures.
                if (!base.empty() && base[0] == '*' && scene != nullptr) {
                    ...
                    if (embIdx >= 0 && embIdx < (int)scene->mNumTextures) {
                        ...
                        std::cout << ">>> [Pato] textura embebida #" << embIdx
                                  << " hint='" << embTex->achFormatHint
                                  << "' size=" << embTex->mWidth << "x" << embTex->mHeight << std::endl;
                        if (embTex->mHeight == 0) {
                            // Datos COMPRIMIDOS (PNG/JPG dentro del FBX): usar SOIL2 desde memoria
                            ...
                            std::cout << ">>> [Pato] embebida comprimida cargada, id=" << texture.id << std::endl;
                        } else {
                            // Datos RAW BGRA (raro pero posible): crear textura directa
                            GLuint tid;
                            glGenTextures(1, &tid);
                            ...
                            texture.id = tid;
                            std::cout << ">>> [Pato] embebida raw cargada, id=" << texture.id << std::endl;
                        }
                    }
                }

                // Caso 2: textura externa (en disco) — buscar archivo por basename + extensiones comunes
                if (texture.id == 0) {
                    const char* exts[] = { "", ".png", ".jpg", ".jpeg", ".tga", ".bmp" };
                    for (const char* ext : exts) {
                        std::string candidate = base + ext;
                        std::string full = this->directory + "/" + candidate;
                        std::ifstream f(full.c_str());
                        if (f.good()) {
                            f.close();
                            std::cout << ">>> [Pato] encontrado en: '" << full << "'" << std::endl;
                            texture.id = TextureFromFile(candidate.c_str(), this->directory);
                            break;
                        }
                    }
                }
                if (texture.id == 0) {
                    std::cout << ">>> [Pato] NO se encontro la textura en " << this->directory << std::endl;
                }
                std::cout << ">>> [Pato] textura id=" << texture.id << " (tipo: " << typeName << ")" << std::endl;
